...
--- !doctype module
--- @class Horizontal : base_component
...
function HorizontalLayout:add_component(cmp_, id)
  ...
  local width_per_item = self.width / (self.num_components + 1)

  print(" Width per item:",width_per_item)
  local offset = 0
...
function HorizontalLayout:new(o)
    self.super.new(self)
...
function HorizontalLayout:setCallback( callback, fn)
  self["__"..callback] = fn
end

function HorizontalLayout:set_size(width, height)
    self.width = width
    self.height = height
...
function HorizontalLayout:set_pos(x, y)
  self.prev_x = self.x
  self.prev_y = self.y
...
function HorizontalLayout:update(clicked,x,y,focused)
  local obj = self
  local redraw = false
  local old =obj.state 
  self:rectangle()
